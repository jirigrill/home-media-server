...
class RadarrService(BaseService):
    ...
    def search_all_missing(self) -> bool:
        ...
        try:
            ...
            self.logger.info(
                f"Starting search for {len(missing_movies)} missing movies"
            )
            ...
            for i, movie in enumerate(missing_movies, 1):
                ...
                self.logger.info(
                    f"Searching for movie {i}/{len(missing_movies)}: {movie_title} (ID: {movie_id})"
                )
                ...
                if i < len(missing_movies):
                    self.logger.info(
                        f"Waiting {self.search_delay_minutes} minutes before searching next movie..."
                    )
                    time.sleep(self.search_delay_minutes * 60)  # Convert minutes to seconds

            self.logger.info(
                f"Completed search cycle for all {len(missing_movies)} missing movies"
            )
            return True
